...
$arItems = $arResult["JS_DATA"]["GRID"]["ROWS"];
	$arAdditives = array();
	foreach($arItems as $key=>$arItem){
		if($arItem["data"]["NOTES"] == "additive"){
			$arAdditives[$arItem["data"]["PRODUCT_ID"]] = $arItem;
			unset($arResult["JS_DATA"]["GRID"]["ROWS"][$key]);
			unset($arItems[$key]);
		}
	}
	//print_arr($arResult['JS_DATA']);
	$fullOrder = "";
	foreach($arItems as $key=>$arItem){
		$propXMLid = CSaleBasket::GetPropsList(array(),array("CODE"=>"PRODUCT.XML_ID","BASKET_ID"=>$key),false,false,array())->GetNext();
		$xml_id = $propXMLid["VALUE"];
		$fullOrder .= $arItem["data"]["NAME"];
		if(strpos($xml_id,"$")!==false){
			$arAdditivesIDs = explode(",",end(explode("$",$xml_id)));
			
			$sum_add = 0;
			
			if(reset($arAdditivesIDs)!="" && count($arAdditivesIDs)>0){
				$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["NOTES"] = "Добавки: ";
				$fullOrder .= ", добавки: ";
			}
			$cnt = 0;
			foreach($arAdditivesIDs as $addID){
				...
				$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["NOTES"] .= $arAdditives[$addID]["data"]["NAME"];
				$fullOrder .= $arAdditives[$addID]["data"]["NAME"];
				if($cnt!= count($arAdditivesIDs)){
					$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["NOTES"] .= ", ";
					$fullOrder .= ", ";
				}
				$sum_add += $arAdditives[$addID]["data"]["PRICE"];
				
			}
			$sum_item = $arItem["data"]["SUM_NUM"];
			$sum = $sum_item+$sum_add;
			$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["SUM_NUM"] = $sum;
			$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["SUM_BASE"] = $sum;
			$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["SUM"] = $sum." руб";
			$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["SUM_BASE_FORMATTED"] = $sum." руб";
		}else{
			$arResult["JS_DATA"]["GRID"]["ROWS"][$key]["data"]["NOTES"] =  "";
		}
		$fullOrder .= "; количество - ".$arItem["data"]["QUANTITY"]." шт\r\n";
	}
	$arResult["JS_DATA"]["FULL_ORDER_TEXT"] = $fullOrder;
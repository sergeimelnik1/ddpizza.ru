...
if (!empty($arResult['ITEMS'])) {
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        $arItem['CATALOG_QUANTITY'] = (
                0 < $arItem['CATALOG_QUANTITY'] && is_float($arItem['CATALOG_MEASURE_RATIO']) ? floatval($arItem['CATALOG_QUANTITY']) : intval($arItem['CATALOG_QUANTITY'])
                );
        ...
        if ($arItem["PREVIEW_PICTURE"]["ID"] > 0) {
            $file = CFile::ResizeImageGet($arItem["PREVIEW_PICTURE"]["ID"], Array("width" => 326, "height" => 245), BX_RESIZE_IMAGE_EXACT, false);
            if (strlen($file["src"]) > 0) {
        ...
        if ($arResult['MODULES']['catalog']) {
            ...
            if (
                    (CCatalogProduct::TYPE_PRODUCT == $arItem['CATALOG_TYPE'] || CCatalogProduct::TYPE_SKU == $arItem['CATALOG_TYPE']) && !empty($arItem['OFFERS'])
            ) {
            ...
            foreach ($arItem["OFFERS"] as $arOffer) {
                ...
                if (count($arOffer["TREE"]) > 0 && $arOffer["TREE"] != false) {
                    foreach ($arOffer["TREE"] as $keyprop => $value) {
                        ...
                        if ($value > 0) {
                            $arItem["SKU_THERE_ARE"][str_replace("PROP_", "", $keyprop)][] = $value;
                        }
                    }
                }

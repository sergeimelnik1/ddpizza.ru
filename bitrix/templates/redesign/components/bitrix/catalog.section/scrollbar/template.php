...
if (count($arResult["ITEMS"]) > 0) {
	//$rand = CAjax::GetComponentID($component->__name, $component->__template->__name);
				...
				foreach ($arResult["ITEMS"] as $arItem) {
					...
					?><div class="item_element good_box inline" id="<?=$strMainID;?>" data-id="<?=$arItem["ID"];?>">
						...
						<div class="hover_box box<? if ($can_buy != "1") { echo ' disabled'; } ?>">
							...
							?></div>
							<div class="img_box">
								<a href="javascript:;" data-fancybox data-src="<?=$arItem["DETAIL_PAGE_URL"];?>?open_popup=Y" class="zoom open_fancybox" rel="gallery"><span class="zoom__text"><?=GetMessage("STUDIOFACT_FAST_VIEW");?></span></a>
							...
							<div class="price_box main_preview_price_<?=$arItem["ID"]?>">
								<div class="fl">
								...
								<? if ($arParams["USE_PRODUCT_QUANTITY"] == 1 && $can_buy == 1) {
									...
									<a href="javascript: void(0);" class="minus">-</a><?
										?><input type="text"
												 ...
												 value="<?=($arItem["CATALOG_MEASURE_RATIO"] ? : "1");?>"
										/><?
								...
								<? if ($can_buy == "1") { ?>
									<div class="buy_box fr">
										<a
											...
											data-price="<?=str_replace(GetMessage("STUDIOFACT_RUB"), '', $arItem["MIN_PRICE"]["PRINT_DISCOUNT_VALUE"]);?>"
								...
								<? } else {
									?><div class="buy_box fr price_box__not-avilable">

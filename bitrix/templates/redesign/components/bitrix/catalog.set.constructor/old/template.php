...
$blockWidth = 100/(1+$countDefSetItems);
?>
<div class="bx_item_set_hor_container_big">
	...
	<div class="bx_item_set_hor">
		<div class="bx_item_set_hor_item plus" style="width:<?=$blockWidth?>%;" data-price="<?=$arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"]?>" data-old-price="<?=$arResult["ELEMENT"]["PRICE_VALUE"]?>" data-discount-diff-price="<?=$arResult["ELEMENT"]["PRICE_DISCOUNT_DIFFERENCE_VALUE"]?>">
			...
			<a class="bx_item_set_linkitem" href="<?=$arResult["ELEMENT"]["DETAIL_PAGE_URL"]?>"><span class="bx_item_set_name"><?=$arResult["ELEMENT"]["NAME"]?></span>
				<span class="bx_item_set_price">
					<strong>
						<?=str_replace(GetMessage("STUDIOFACT_RUB"), '<span class="rub">'.GetMessage("STUDIOFACT_R").'</span>', $arResult["ELEMENT"]["PRICE_PRINT_DISCOUNT_VALUE"]);?>
					</strong>
				</span>
				<?if (!($arResult["ELEMENT"]["PRICE_VALUE"] == $arResult["ELEMENT"]["PRICE_DISCOUNT_VALUE"])):?><span class="bx_item_set_price old"><strong><?=$arResult["ELEMENT"]["PRICE_VALUE"]?> <span class="rub"><?= GetMessage("STUDIOFACT_R"); ?></span></strong></span><?endif?>
			</a>
		...
		<?foreach($arResult["SET_ITEMS"]["DEFAULT"] as $key => $arItem):?>
			<div class="bx_item_set_hor_item <?if ($key<$countDefSetItems-1) echo "plus"; else echo "equally"?> bx_default_set_items"
				...
				<a class="bx_item_set_linkitem" href="<?=$arItem["DETAIL_PAGE_URL"]?>"><span class="bx_item_set_name"><?=$arItem["NAME"]?></span>

				</a>
				<div class="bx_item_set_price">
					<strong>
						<?=str_replace(GetMessage("STUDIOFACT_RUB"), '<span class="rub">'.GetMessage("STUDIOFACT_R").'</span>', $arItem["PRICE_PRINT_DISCOUNT_VALUE"]);?>
					</strong>
				</div>
				<span  class="bx_item_set_price old" <?if ($arItem["PRICE_VALUE"] == $arItem["PRICE_DISCOUNT_VALUE"]):?>style="display:none"<?endif?>>
					<strong>
						<?=str_replace(GetMessage("STUDIOFACT_RUB"), '<span class="rub">'.GetMessage("STUDIOFACT_R").'</span>', $arItem["PRICE_PRINT_VALUE"]);?>
					</strong>
				</span>
				<span class="bx_item_set_del" onclick="catalogSetDefaultObj_<? echo $intElementID; ?>.DeleteItem(this.parentNode, '<?=$arItem["ID"]?>')">Удалить из набора</span>
		...
		<div class="bx_item_set_hor_item result">
			<span class="bx_item_set_result_block">
				<?if ($arResult["SET_ITEMS"]["OLD_PRICE"]):?>
					<div class="bx_item_set_old_price"><?=$arResult["SET_ITEMS"]["OLD_PRICE"]?></div>
				<?endif?>
				<div class="bx_item_set_current_price">
					<?=str_replace(GetMessage("STUDIOFACT_RUB"), '<span class="rub">'.GetMessage("STUDIOFACT_R").'</span>', $arResult["SET_ITEMS"]["PRICE"]);?>
				</div>
			...
			<a href="javascript:void(0)" onclick="catalogSetDefaultObj_<? echo $intElementID; ?>.Add2Basket();" class="bx_bt_white bx_medium button detail_buy_button"><span class="text"><?=GetMessage("CATALOG_SET_BUY")?></span></a>
		</div>

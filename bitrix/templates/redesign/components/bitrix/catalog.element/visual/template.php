...
<div class="bx_item_detail box padding good_box" id="<?=$itemIds['ID']?>" itemscope itemtype="http://schema.org/Product" data-product-id="<?=!empty($arResult['OFFERS']) ? $arResult['OFFER_ID_SELECTED'] : $arResult['ID']?>">
	<div class="row product-wrapper">
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 product-images-wrapper" id="main-slider">
		...
		<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 product-description-wrapper">
			...
			<div class="product-options-wrapper">
				...
				if ($haveOffers && !empty($arResult['OFFERS_PROP']))
				{
								...
								<ul class="product-item-scu-item-list">
									...
									foreach ($skuProperty['VALUES'] as &$value)
									{
										...
										{	
											...
											if($arOffer["TREE"]["PROP_".$propertyId] == $value['ID']){
												$rounded_price = $arOffer["PRICES"]["BASE"]["ROUND_VALUE_VAT"];
												break;
											}
										}
										...
										else
										{
											...
											<li class="product-item-scu-item-text-container offer_sku" title="<?=$value['NAME']?>"
											    data-treevalue="<?=$propertyId?>_<?=$value['ID']?>"
											    data-onevalue="<?=$value['ID']?>" data-price="<?=$rounded_price?>">
					...
					{
						?>
						<div class="detail_preview_text main_detail_preview_text" itemprop="description" style="display: block;" id="offer_preview_text_<?=$arResult['ID']?>">
	...
	<div class="clearfix"></div>
	<?if(count($arResult["ADDITIVES_SECTIONS"])>0){?>
	<div class="product-divider"></div>
		...
		<? foreach($arResult["ADDITIVES_SECTIONS"] as $arSect){
			...
			<div class="tabs_head <?=$is_active_tabs?>">
				<a id="additives_<?=$arSect["ID"]?>" href="#" data-href=".dt<?=$tab_cnt?>" title="<?=$arSect["NAME"]?>">
					<span class="icon_description icons_head"></span>
					<span class="text"><?=$arSect["NAME"]?></span>
				</a>
			</div>
		<?}?>
		...
		?>
			<? foreach($arResult["ADDITIVES_SECTIONS"] as $arSect){
				...
				?>
				<div class="tabs_body product-additives-wrapper dt<?=$tab_cnt?> <?=$is_active_content?>">
                    <div class="offers_item">
						<div class="offer_item" data-id="<?=$arSect["ID"]?>">
							<ul class="product-item-scu-item-list">
								<?foreach($arResult["ADDITIVES"][$arSect["ID"]] as $arAdd){
									
									?>
									<li class="product-item-scu-item-text-container offer_sku additive" data-id="<?=$arAdd["ID"]?>" data-price="<?=intval($arAdd["PRICE"])?>">
										<div class="product-item-scu-item-text"><?=$arAdd["NAME"]?> <span>+ <?=intval($arAdd["PRICE"])?> руб</span></div>
									</li>
								<?}?>								
							</ul>
						</div>
...
<div class="clearfix clr clear cl"></div>
<?
...
<script>
	BX.message({
		...
		BTN_MESSAGE_CLOSE_POPUP_2: '<?=GetMessageJS('CT_BCE_CATALOG_BTN_MESSAGE_CLOSE_POPUP')?>',

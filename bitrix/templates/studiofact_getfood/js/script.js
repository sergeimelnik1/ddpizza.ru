...
$(document).ready(function(){
	...
	setInterval(function() {$(".bx-soa-customer .bx-soa-customer-field").each(function(){
		...
		$(".topMenuSmall").mouseleave(function(){
			$(this).slideUp();
		});
		$(".basket_action_container").click(function(){
			var $flyingPic=$(".flying_pic");
			var new_y=$("#small_basket").offset().top+30;
			var new_x=$("#small_basket").offset().left+30;

			//$flyingPic.html("");
			$flyingPic.removeAttr("style");
			$flyingPic.stop(true);
			$flyingPic.css("top",($(this).offset().top-160)+"px");
			$flyingPic.css("left",($(this).offset().left-$(".catalog_section").offset().left+60)+"px");
			$flyingPic.css("width",$(this).closest(".item_element").width()+"px");

			$(".flying_pic img").attr("src",$(this).closest(".item_element").find(".product-item-image-original").data("src"));
			var old_w=$flyingPic.width();
			$flyingPic.animate({top: new_y+"px",left: new_x+"px",width: "40px"}, 300).delay(300).animate({width: 0}, 300);

...
if (!empty($arResult['ITEMS']))
{
	...
	{
        //формируем CAN_BUY для подарка
        if(empty($arItem["OFFERS"])){
            $arPiceCode = array();
            foreach($arItem["PRICES"] as $k => $val)
                $arPiceCode[] = $k;
            $arCatalogPrices = CIBlockPriceTools::GetCatalogPrices($arItem["IBLOCK_ID"], array("BASE"));
            $arrCanBuy = CIBlockPriceTools::CanBuy($arItem["IBLOCK_ID"], $arCatalogPrices, $arItem);
            $arResult['ITEMS'][$key]["CAN_BUY"] = $arrCanBuy;
        }else{
            foreach($arItem["OFFERS"] as $arOffer){
                $arPiceCode = array();
                foreach($arOffer["PRICES"] as $k => $val)
                    $arPiceCode[] = $k;
                $arCatalogPrices = CIBlockPriceTools::GetCatalogPrices($arItem["IBLOCK_ID"], array("BASE"));
                $arrCanBuy = CIBlockPriceTools::CanBuy($arItem["IBLOCK_ID"], $arCatalogPrices, $arOffer);
                $arResult['ITEMS'][$key]["OFFER"][$k]["CAN_BUY"] = $arrCanBuy;
            }
        }

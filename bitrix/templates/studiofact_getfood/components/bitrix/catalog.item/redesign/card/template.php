...
<div class="img_box">
	<a class="product-item-image-wrapper" href="javascript:void(0)<?//=$item['DETAIL_PAGE_URL']?>" title="<?=$imgTitle?>" data-entity="image-wrapper">
		...
		<span class="product-item-image-original" id="<?=$itemIds['PICT']?>" data-src="<?=$item['PREVIEW_PICTURE']['SRC']?>" 
		...
		//print_arr($item['PROPERTIES']);
		if (!empty($item['PROPERTIES']["BRAND_REF"]["VALUE"]))
		{
   ...
   $res = $entityClass::getList(array(
       'select' => array('*'),
       //'order' => array('ID' => 'ASC'),
       'filter' => array('UF_XML_ID' => $item['PROPERTIES']["BRAND_REF"]["VALUE"])
   ));
   $arMarks = array();
   while($propVal = $res->fetch()){
	   $arMarks[] = $propVal;
   }
			...
			<div class="icon_box product-item-label-text <?=$labelPositionClass?>" id="<?=$itemIds['STICKER_ID']?>">
				...
				if (!empty($arMarks))
				{
					foreach ($arMarks as $propVal)
					{
						?>
						<div class="<?=strtolower($propVal["UF_XML_ID"])?>" title="<?=$propVal["UF_NAME"]?>"><img src="<?=CFile::GetPath($propVal["UF_FILE"])?>" alt="<?=$propVal["UF_NAME"]?>" /></div>
						<?
					}
				}
				?>
			</div>
			<span class="triangle tr_small"></span>
			<?
		}
		?>
	...
	<?/*<div class="hover_over">
		<a href="javascript:;" data-width="810" data-fancybox data-src="<?=$item["DETAIL_PAGE_URL"];?>?open_popup=Y" class="open_fancybox" >
...
<div class="item-info">
	<div class="bx_catalog_item_title name">
		<a href="javascript:void(0)<?//=$item['DETAIL_PAGE_URL']?>" title="<?=$productTitle?>"><?=$productTitle?></a>
	</div>
	<div class="shortdesc">
		<?=$item["PREVIEW_TEXT"]?>
	</div>

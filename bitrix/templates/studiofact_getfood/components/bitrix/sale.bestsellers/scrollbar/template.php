...
if (count($arResult["ITEMS"]) > 0) {
	...
	?>
	<div class="viewed-profile-block bestsellers-slider-block">

		<div class="section_box ">
			<? if (strlen($arParams["SECTION_NAME"]) > 0) { ?><h3 class="product-categories-title"><?=$arParams["SECTION_NAME"];?></h3><? } ?>
					...
					foreach ($arResult["ITEMS"] as $arItem) {
						...
						<div class="hover_box box<? if ($can_buy != "1") { echo ' disabled'; } ?>">
							<div class="icon_box"><?
								...
								?></div>
							<div class="img_box">
								<a href="javascript:;" data-fancybox="group3" data-src="<?=$arItem["DETAIL_PAGE_URL"];?>?open_popup=Y" class="zoom open_fancybox" rel="gallery"><span class="zoom__text"><?=GetMessage("STUDIOFACT_FAST_VIEW");
?></span></a>
							...
							<a href="<?=$arItem["DETAIL_PAGE_URL"];?>" title="<?=$arItem["NAME"];?>" class="name"><?=$arItem["NAME"];
?></a>
							<div class="price_box">
								<div class="fl">
									...
									<div class="price_box__actual-price"><? if (count($arItem["OFFERS"]) > 0) { echo GetMessage("SF_ISSET_OFFERS"); } ?><?=str_replace(GetMessage("STUDIOFACT_RUB"), '<span class="rub">'.GetMessage("STUDIOFACT_R").'</span>', $arItem["MIN_PRICE"]["PRINT_DISCOUNT_VALUE"]);
?></div>
								</div>
										...
										if (count($arSku["VALUES"]) > 0 && count($arItem["SKU_THERE_ARE"][$arSku["ID"]]) > 0) {
											...
											foreach ($arSku["VALUES"] as $value) {
												if ($value["ID"] > 0 && in_array($value["ID"], $arItem["SKU_THERE_ARE"][$arSku["ID"]])) {
													?><span class="offer_sku" data-prop-id="<?=$arSku["ID"];?>" data-prop-code="<?=$arSku["CODE"];?>" data-prop-value-id="<?=$value["ID"];?>" data-tree='<?=json_encode($arItem["SKU_TREE"]);?>'><?=(strlen($value["PICT"]["SRC"]) > 0?'<img src="'.$value["PICT"]["SRC"].'" title="'.$value["NAME"].'" alt="'.$value["NAME"].'" />':$value["NAME"]);
?></span><?
												}
											}
											echo '</div>';
								...
								<div class="price_box main_preview_price offers_hide">
									<div class="pr">
										<span class="price"><? if (count($arItem["OFFERS"]) > 0) { echo GetMessage("SF_ISSET_OFFERS"); } ?><?=str_replace(GetMessage("STUDIOFACT_RUB"), '<span class="rub">'.GetMessage("STUDIOFACT_R").'</span>', $arItem["MIN_PRICE"]["PRINT_DISCOUNT_VALUE"]);
?></span>
...
?></a><a href="javascript: void(0);" class="button_white"><?=GetMessage("SF_GO_TO_BACK_BUTTON");
?></a></div>
									<? } else {
										?><div class="nav_buttons"><?if (strlen($arParams["MESS_NOT_AVAILABLE"]) > 0) { echo $arParams["MESS_NOT_AVAILABLE"]; } else { echo GetMessage("CT_BCE_CATALOG_NOT_AVAILABLE"); } ?><a href="javascript: void(0);" class="button_white"><?=GetMessage("SF_GO_TO_BACK_BUTTON");
?></a></div><?
									} ?>
								</div>
								<? if (count($arItem["OFFERS"]) > 0) {

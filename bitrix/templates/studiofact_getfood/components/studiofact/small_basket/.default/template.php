...
foreach($arResult[["BASKET_ITEMS"] as $key=>$arItem){
	...
	foreach($arAdditivesIDs as $add_id){
		$arResult[["BASKET_ITEMS"][$key]["ADDITIVES"][$add_id] = $arAdditives[$add_id];
	}
}
foreach($arResult[["BASKET_ITEMS"] as $key=>$arItem){
	...
	$arResult[["BASKET_ITEMS"][$key]["FULL_PRICE"] = $price;
	$arResult[["BASKET_ITEMS"][$key]["FULL_PRICE_FORMATED"] = number_format($price,0,""," ").' '.'<span class="rub black">Р</span>';
	$arResult[["BASKET_ITEMS"][$key]["SUM_VALUE"] = $price*$arItem["QUANTITY"];
	$arResult[["BASKET_ITEMS"][$key]["SUM"] = number_format($price*$arItem["QUANTITY"],0,""," ").' '.'<span class="rub black">Р</span>';
	
	$price = $arItem["PRICE"];
	foreach($arItem["ADDITIVES"] as $add_id=>$arAdditive){
		$price += $arAdditive["PRICE"];
	}
	$arResult[["BASKET_ITEMS"][$key]["PRICE"] = $price;
	$arResult[["BASKET_ITEMS"][$key]["PRICE_FORMATED"] = number_format($price,0,""," ").' '.'<span class="rub black">Р</span>';

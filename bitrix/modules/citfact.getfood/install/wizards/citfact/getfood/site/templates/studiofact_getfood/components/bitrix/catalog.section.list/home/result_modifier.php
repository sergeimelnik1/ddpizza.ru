...
if (0 < $arResult['SECTIONS_COUNT'])
{
	if ('LIST' != $arParams['VIEW_MODE'])
	{
      ...
      if ($arOneSection["PICTURE"]["ID"] > 0) {
        $file = CFile::ResizeImageGet($arOneSection["PICTURE"]["ID"], Array("width" => 325, "height" => 173), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, false);
        if (strlen($file["src"]) > 0) {
          $arOneSection["PICTURE"]["ORIGINAL_SRC"] = $arOneSection["PICTURE"]["SRC"];
          $arOneSection["PICTURE"]["SRC"] = $file["src"];
        }
      }
		}
		unset($arOneSection);

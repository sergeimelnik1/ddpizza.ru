...
class SaleBasketLineComponent extends CBitrixComponent
{
	...
	private function getProducts()
	{
			...
			if ($item['CAN_BUY'] == 'Y')
			{
				if ($item['DELAY'] == 'Y')
					...
					if($item["NOTES"]!="additive"){
						...
						if(count($arAdditivesIDs)>0 && $arAdditivesIDs[0]!=NULL){
							$arItemPropValues = array();
							foreach($arAdditivesIDs as $arAdditivesID){
								$arPrice = CCatalogProduct::GetOptimalPrice($arAdditivesID,1,$USER->GetUserGroupArray(),"N",array(),"s1");
	
								$item["PRICE"]+=$arPrice["DISCOUNT_PRICE"];
								$arItemPropValues[] = $arAdditives[$arAdditivesID]["NAME"];
							}
							if(!empty($arItemPropValues) && $arItemPropValues[0]!=NULL){
								
								$item["CUSTOM_PROPS"]["ADDITIVES"] = array("NAME"=>"Добавки","VALUE"=>implode(",",$arItemPropValues));
								$item["PRICE_FORMATED"] = number_format($item["QUANTITY"]*$item["PRICE"],0,""," ")." руб";
							}
						}
		...
		$result['NUM_PRODUCTS'] = $non_additive_cnt;

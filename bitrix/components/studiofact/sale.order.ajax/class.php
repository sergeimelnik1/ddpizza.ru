	...
	foreach($arItems as $key=>$arItem){
		...
		if(strpos($xml_id,"$")!==false){
			...
			if(reset($arAdditivesIDs)!="" && count($arAdditivesIDs)>0){
				$result["GRID"]["ROWS"][$key]["data"]["NOTES"] = "Добавки: ";
				$fullOrder .= ", добавки: ";
			}
			$cnt = 0;
			foreach($arAdditivesIDs as $addID){
				...
				$result["GRID"]["ROWS"][$key]["data"]["NOTES"] .= $arAdditives[$addID]["data"]["NAME"];
				$fullOrder .= $arAdditives[$addID]["data"]["NAME"];
				if($cnt!= count($arAdditivesIDs)){
					$result["GRID"]["ROWS"][$key]["data"]["NOTES"] .= ", ";
					$fullOrder .= ", ";
				}
				$sum_add += $arAdditives[$addID]["data"]["QUANTITY"]*$arAdditives[$addID]["data"]["PRICE"];
				
			}
			
			$sum_item = $arItem["data"]["SUM_NUM"];
			$sum = $sum_item+$sum_add;
			$result["GRID"]["ROWS"][$key]["data"]["SUM_NUM"] = $sum;
			$result["GRID"]["ROWS"][$key]["data"]["SUM_BASE"] = $sum;
			$result["GRID"]["ROWS"][$key]["data"]["SUM"] = $sum." руб";
			$result["GRID"]["ROWS"][$key]["data"]["SUM_BASE_FORMATTED"] = $sum." руб";
		}else{
			$result["GRID"]["ROWS"][$key]["data"]["NOTES"] =  "";
		}
		$fullOrder .= "; количество - ".$arItem["data"]["QUANTITY"]." шт\\r\\n";
	}
	$result["FULL_ORDER"] = $fullOrder;
	//$arUserResult = &$this->arUserResult;
	//$arUserResult["USER_PROPS_N"][8]["VALUE"]  = $fullOrder;

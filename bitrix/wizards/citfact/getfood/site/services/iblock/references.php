...
if (!$dbHblock->Fetch())
{
	$data = array(
		'NAME' => 'ColorReference',
		'TABLE_NAME' => 'citfact_color_reference_delivery_' . WIZARD_SITE_ID,
	);
    if(Application::getConnection()->isTableExists($data['TABLE_NAME'])){
        Application::getConnection()->query(sprintf(
            "INSERT INTO b_hlblock_entity (NAME, TABLE_NAME) VALUES ('%s', '%s');",
            $data["NAME"],
            $data["TABLE_NAME"]
        ));
        $result = Application::getConnection()->query('SELECT LAST_INSERT_ID()')->fetch();
        if(is_array($result) && !empty($result['LAST_INSERT_ID()'])){
...
if (!$dbHblock->Fetch())
{
	$data = array(
		'NAME' => 'BrandReference',
		'TABLE_NAME' => 'citfact_brand_reference_delivery_' . WIZARD_SITE_ID,
	);
    if(Application::getConnection()->isTableExists($data['TABLE_NAME'])){
        Application::getConnection()->query(sprintf(
            "INSERT INTO b_hlblock_entity (NAME, TABLE_NAME) VALUES ('%s', '%s');",
